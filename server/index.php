...
$ws_worker->onMessage = function ($connection, $data) use(&$users) {
    ...
    if(isset($data['sender']) && isset($data['msg']) && isset($data['receiver'])){

        $message = json_encode(['user' => $data['sender'], 'msg' => $data['msg']]);

        if($data['receiver'] === 'all'){
            foreach($users as $user=>$connection){
        ...
        if(isset($users[$data['receiver']])){
            $users[$data['receiver']]->send($message);
        }
